...
class UnifiedModelInterface:
    ...
    def list_available_models(self) -> Dict:
        ...
        for provider, models in self.models.items():
            ...
            for model_name, config in models.items():
                ...
                status = (
                    "✅"
                    if self._check_model_availability(provider, model_name)
                    else "⏳"
                )
    ...
    def query_cloud_model(self, provider: str, model: str, prompt: str) -> str:
        ...
        if provider == "gemini":
            ...
            return self._query_xai(model, prompt, api_key)
        else:
    ...
    def _query_xai(self, model: str, prompt: str, api_key: str) -> str:
        """Запрос к xAI (Grok) API через requests."""
        try:
            import requests
    ...
    def query(
        self, model: str, prompt: str, provider: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if not provider:
            return {"status": "error", "error": f"Model {model} not found"}
    ...
    def _save_to_cache(self, result: Dict):
        ...
        cache_file = (
            self.cache_dir
            / f"{result['model']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
if __name__ == "__main__":
    ...
    result = interface.query(
        model="gemma:2b", prompt="Кратко объясни что такое Machine Learning"
    )
